...
CASES = [
    ...
    ('sec-col-add-tail', '表：末尾に列を追加', 'BEFORE は4列。AFTER は末尾に「備考」列が増えて5列。既存列の値は同じ。', sec_col_add_tail),
    ('sec-col-add-mid', '表：途中に列を追加', 'BEFORE は4列。AFTER は「設置場所」と「数量」の間に「管理担当」列が増えて5列。', sec_col_add_mid),
    ('sec-col-del-mid', '表：途中の列を削除', 'BEFORE は5列。AFTER は「取得年度」列が無くなって4列。前後の列は同じ。', sec_col_del_mid),
    ('sec-col-del-tail', '表：末尾の列を削除', 'BEFORE は5列。AFTER は末尾の「旧管理番号」列が無くなって4列。', sec_col_del_tail),
    ('sec-row-churn', '表：行の増減と値の変更', 'BEFORE にだけ Classic 行、AFTER にだけ Pro Plus 行。金額も6箇所違う。', sec_row_churn),
    ('sec-lookalike', '表：似た行が並ぶ中での1行削除', 'BEFORE は8行、AFTER は sato.k のログイン行が無くて7行。tanaka.r の設定変更行の結果も違う。', sec_lookalike),
    ('sec-cards', 'カード：追加・削除・並べ替え', 'BEFORE にだけ「通知基盤の移行」、AFTER にだけ「データ基盤のPoC」。「決済APIの連携」は位置も本文も違う。', sec_cards),
    ('sec-card-attr', 'カード：classだけの変更', 'テキストは完全に同一。class が BEFORE は card、AFTER は card done。', sec_card_attr),
    ('sec-list-nest', '入れ子リストの組み替え', 'BEFORE と AFTER で階層・並び順・ul/ol が違う。BEFORE にだけ「社内レビュー」。', sec_list_nest),
    ('sec-steps', '手順の並べ替えと本文書き換え', '「障害対応チームの招集」が BEFORE では2番目、AFTER では4番目。「暫定対処の実施」の説明文も違う。', sec_steps),
    ('sec-glossary', '用語集：用語の増減と定義の書き換え', 'AFTER にだけ SLO、BEFORE にだけ オンコール。エスカレーションの定義文も違う。', sec_glossary),
    ('sec-prose', '長文：ごく小さな修正のみ', '6段落のうち BEFORE と AFTER で違うのは3箇所だけ。過剰にマークしないことの確認。', sec_prose),
    ('sec-inline', 'インライン記法・属性のみの変更', '可視テキストは BEFORE と AFTER で完全に同一。strong / em / href / class だけが違う。', sec_inline),
    ('sec-code', 'コードブロックの編集', '1行が違い、AFTER にだけある行と BEFORE にだけある行が1本ずつ。インデントが崩れないことの確認。', sec_code),
    ('sec-quote', '引用文の部分修正', 'blockquote の数値と語尾が BEFORE と AFTER で違う。', sec_quote),
    ('sec-hlevel', '見出しレベルの昇格', 'テキストは同一。「データの流れ」が BEFORE は h3、AFTER は h2。', sec_hlevel),
    ('sec-swap', '小見出しの入れ替え', '2つの小見出しブロックの前後が BEFORE と AFTER で逆。', sec_swap),
]
